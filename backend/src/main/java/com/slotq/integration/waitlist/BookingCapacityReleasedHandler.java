...
@Component
public class BookingCapacityReleasedHandler implements EventHandler {
    public static final ConsumerRoute ROUTE = new ConsumerRoute("waitlist.promotion", "booking.capacity-released", 1);
    private final WaitlistPromotionEventAdapter adapter;
    BookingCapacityReleasedHandler(WaitlistPromotionEventAdapter adapter) { this.adapter = adapter; }
    @Override public ConsumerRoute route() { return ROUTE; }
    @Override @Transactional(propagation = Propagation.MANDATORY)
    public void handle(StoredEvent event) {
        adapter.handle(event, ROUTE, WaitlistPromotionUseCase.Signal.CAPACITY_RELEASED);
    }
}
